...
* A validation *requestHash* is a global key: the first `validationRequest` to use a given hash claims it, and a later request reusing it reverts. A party that observes a pending request can therefore front-run it with the same hash under an agent it controls, blocking the intended request. Because the requester controls the request payload, it SHOULD include a unique salt (for example a nonce or timestamp) so that `requestHash` is unpredictable and cannot be squatted.
