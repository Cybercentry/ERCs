...
| --- | --- |
| `IIdentityRegistry` | `0x2ab89dcb` |
| `IReputationRegistry` | see note |
| `IValidationRegistry` | `0x092145c2` |

Identifiers are the XOR of the four-byte selectors of the functions declared in each interface, excluding those inherited from ERC-721. The `IReputationRegistry` identifier is omitted here pending the paginated signatures being settled; it MUST be recomputed from the final interface.
...
* Then `getAgentValidations(agentId)` returns both request hashes
* And `getValidatorRequests(0xV1)` returns only the request addressed to `0xV1`
