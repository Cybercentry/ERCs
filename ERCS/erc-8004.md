...
* Given `0xA` owns `agentId`, and `0xW` signs the EIP-712 `AgentWalletSet(agentId, newWallet, owner, deadline)` struct
* When `0xA` calls `setAgentWallet(agentId, 0xW, deadline, signature)` with `block.timestamp <= deadline`
* Then `getAgentWallet(agentId) == 0xW` and `MetadataSet(agentId, "agentWallet", "agentWallet", 0xW)` is emitted
...
**RR-5: `getSummary` requires clientAddresses and averages matching feedback**

* Given `0xC1` gave `80` and `90`, and `0xC2` gave `100`, all with `tag1 = "service"`, `tag2 = "fast"`, `valueDecimals = 0`
* When `getSummary(agentId, [0xC1, 0xC2], "service", "fast")` is called
* Then it returns `count = 3` and `summaryValue = 90`
