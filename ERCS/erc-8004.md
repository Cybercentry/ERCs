...
interface IReputationRegistry {
    ...
    function getSummary(uint256 agentId, address[] calldata clientAddresses, string calldata tag1, string calldata tag2, uint64 offset, uint64 limit) external view returns (uint64 count, int256 sumWad, uint64 total);
    function readAllFeedback(uint256 agentId, address[] calldata clientAddresses, string calldata tag1, string calldata tag2, bool includeRevoked, uint64 offset, uint64 limit) external view returns (address[] memory clients, uint64[] memory feedbackIndexes, int128[] memory values, uint8[] memory valueDecimals, string[] memory tag1s, string[] memory tag2s, bool[] memory revokedStatuses, uint64 total);
    function getClients(uint256 agentId, uint64 offset, uint64 limit) external view returns (address[] memory clients, uint64 total);
}
...
```solidity
function getSummary(uint256 agentId, address[] calldata clientAddresses, string tag1, string tag2, uint64 offset, uint64 limit) external view returns (uint64 count, int256 sumWad, uint64 total)
...
// count is the number of entries in this page; total is the number of ALL matching entries.
// The registry returns a sum, not an average, deliberately. A sum has exactly one correct
// answer, so independent implementations agree on it; and sums compose across pages, so a
// consumer may page a large result set and add the parts. Any averaging, weighting or decay
// is the consumer's choice, consistent with the Rationale below.
...
// offset skips that many matching entries; limit caps how many are returned; total counts ALL matching entries.
// Results MUST be ordered by client, in the order clients first gave feedback, then by ascending feedbackIndex,
// so that paging is stable. Implementations MUST bound the work performed to the requested page: these reads are
// declared for on-chain consumption, and an unbounded scan is unusable past a few thousand entries and cheaply
// griefable, since anyone may add feedback to an agent they do not own.
...
**RR-5: `getSummary` requires clientAddresses and returns a composable sum**

* Given `0xC1` gave `80` and `90`, and `0xC2` gave `100`, all with `tag1 = "service"`, `tag2 = "fast"`, `valueDecimals = 0`
* When `getSummary(agentId, [0xC1, 0xC2], "service", "fast", 0, 100)` is called
* Then it returns `count = 3`, `sumWad = 270 * 10^18`, and `total = 3`
* And the consumer computes the mean itself: `sumWad / count = 90 * 10^18`
* When `clientAddresses` is empty, the call reverts
* An empty `tag1` or `tag2` matches any value for that tag

**RR-5b: `getSummary` composes across pages**

* Given the same three entries
* When `getSummary(agentId, [0xC1, 0xC2], "service", "fast", 0, 2)` returns `count = 2`, `sumWad = 170 * 10^18`, `total = 3`
* And `getSummary(agentId, [0xC1, 0xC2], "service", "fast", 2, 2)` returns `count = 1`, `sumWad = 100 * 10^18`, `total = 3`
* Then the sums and counts add to the unpaged result: `270 * 10^18` over `3` entries
* This is why the registry returns a sum: an average cannot be recombined from pages

**RR-6: `readAllFeedback` filters by client, tag, and revocation**

* Given `0xC` gave `70` (`tag1 = "quality"`), `80` (`tag1 = "speed"`), and `90` (`tag1 = "reliability"`)
* When `readAllFeedback(agentId, [0xC], "", "", false)` is called
* Then it returns three entries with `values = [70, 80, 90]` and `tag1s = ["quality", "speed", "reliability"]`
